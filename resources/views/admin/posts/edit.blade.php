...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.posts.update", [$post->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="picture">{{ trans('cruds.post.fields.picture') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.post.fields.picture_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="authors">{{ trans('cruds.post.fields.author') }}</label>
                ...
                <select class="form-control select2 {{ $errors->has('authors') ? 'is-invalid' : '' }}" name="authors[]" id="authors" multiple>
                    @foreach($authors as $id => $author)
                        <option value="{{ $id }}" {{ (in_array($id, old('authors', [])) || $post->authors->contains($id)) ? 'selected' : '' }}>{{ $author }}</option>
                    @endforeach
                </select>
...
@if(isset($post) && $post->picture)
      var file = {!! json_encode($post->picture) !!}
